...
contract Voter is IVoter {
    ...
    function createGauge(address _pool, uint256 _gaugeType) external returns (address) {
        ...
        if (isPair) {
            ...
            // if one of the tokens is not base (KUS) then add base(KUS) to allowed rewards
            if (base != tokenA && base != tokenB) {
              allowedRewards[2] = base;
            }
        }

        if (msg.sender != governor && msg.sender != emergencyCouncil) { // gov can create for any pool, even non-Kuswap pairs
