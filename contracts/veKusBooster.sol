...
contract veKusBooster is Ownable {
    ...
    constructor(address _voting_escrow, address _team, address _paymentToken, uint256 _maxLock, address _router) {
        ...
        kus = IVotingEscrow(voting_escrow).token();
    ...
    function balanceOfKus() public view returns (uint){
        return IERC20(kus).balanceOf(address(this));
    }

    function maxLockableAmount() public view returns (uint){
         uint256 kusBal = balanceOfKus();
         uint256 amnt = kusBal * 100 / matchRate;
         return amnt;
    }

    function checkKusBalanceEnough(uint256 _paymentAmount) public view returns (bool) {
        uint256 amount = IRouter(router).getAmountOut(_paymentAmount, paymentToken, kus, false);
        return balanceOfKus() >= amount * matchRate  / 100;
    }
    ...
    function boostedBuyAndVeLock(uint256 _amount, uint _minOut) public {
        ...
        require(balanceOfKus() > 0, 'no extra KUS for boosting');
        ...
        uint256 kusBefore = balanceOfKus();
        IRouter(router).swapExactTokensForTokensSimple(_amount, _minOut, paymentToken, kus, false, address(this), block.timestamp);
        uint256 kusAfter = balanceOfKus();
        uint256 kusResult = kusAfter - kusBefore;

        uint256 amountToLock = kusResult * matchRate  / 100 + kusResult;
    ...
    function donateKus(uint256 _amount) public {
        require(_amount > 0, 'need to add at least 1 KUS');
        IERC20(kus).transferFrom(msg.sender, address(this), _amount);
        _giveAllowances();
